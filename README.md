...
Simply run the app as a Spring Boot application with the `dev` Spring profile. Use whatever IDE tool you want to help you with this. Note, I used to simply run `mvn spring-boot:run -Dspring-boot.run.profiles=dev -Dspring-boot.run.fork=false`; however `spring-boot.run.fork` was removed in spring boot 3. Now, I use the Spring Boot Dashboard extension in VSCode to run my app with the `dev` profile.
