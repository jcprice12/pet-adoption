...
public class S3Service implements IS3Service {
    ...
    public String uploadMultipartFileToS3(MultipartFile multipartFile, String keyPrefix) throws UploadS3FileException {
        try {
            ...
            String key = String.format("pets/%s/%d_%s", keyPrefix, System.currentTimeMillis(),
    ...
    private Credentials createTemporaryCredentials() {
        String roleSessionName = String.format("%d_%s", System.currentTimeMillis(),
                SecurityContextHolder.getContext().getAuthentication().getName());
        AWSSecurityTokenService stsClient = AWSSecurityTokenServiceClientBuilder.standard()
                .withCredentials(new EnvironmentVariableCredentialsProvider()).withRegion(Regions.US_EAST_2).build();
        AssumeRoleRequest roleRequest = new AssumeRoleRequest().withRoleArn(ROLE_ARN)
                .withRoleSessionName(roleSessionName).withDurationSeconds(SESSION_DURATION_IN_SECONDS);
        return stsClient.assumeRole(roleRequest).getCredentials();
    }

    private AmazonS3 createS3Client(Credentials creds) {
        BasicSessionCredentials awsCredentials = new BasicSessionCredentials(creds.getAccessKeyId(),
